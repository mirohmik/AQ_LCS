...
malfunc.filter <- function(sensorunit){
    ...
    ### NO_00
    {
      ...
      for (i in period_for_sensor)
      {
        for(j in 1:nrow(data_NO_00))
        {
          ...
          {
            data_NO_00$Status[j] <-1
          }
        }
      }
      data_NO_00_nonfilter <- data_NO_00 %>% filter(Status != 1)
      data_NO_00_filter <- data_NO_00 %>% filter(Status == 1)
    ...
    ### NO_01
    {
      ...
      for (i in period_for_sensor)
      {
        for(j in 1:nrow(data_NO_01))
        {
          ...
          {
            data_NO_01$Status[j] <-1
          }
        }
      }
      data_NO_01_nonfilter <- data_NO_01 %>% filter(Status != 1)
      data_NO_01_filter <- data_NO_01 %>% filter(Status == 1)
      data_NO_01$Status <- as.character(data_NO_01$Status)
    }
    ### NO2_00
    {
      data_NO2_00$Status <-0
      for (i in period_for_sensor)
      {
        for(j in 1:nrow(data_NO2_00))
        {
          ...
          {
            data_NO2_00$Status[j] <-1
          }
        }
      }
      data_NO2_00_nonfilter <- data_NO2_00 %>% filter(Status != 1)
      data_NO2_00_filter <- data_NO2_00 %>% filter(Status == 1)
      data_NO2_00$Status <- as.character(data_NO2_00$Status)
    }
    ### NO2_01
    {
      data_NO2_01$Status <-0
      for (i in period_for_sensor)
      {
        for(j in 1:nrow(data_NO2_01))
        {
          ...
          {
            data_NO2_01$Status[j] <-1
          }
        }
      }
      data_NO2_01_nonfilter <- data_NO2_01 %>% filter(Status != 1)
      data_NO2_01_filter <- data_NO2_01 %>% filter(Status == 1)
  ...
  ## Plotting scatter plot which involved in the supplementary materials ------
  {
     pdata_NO_A_RLM <- ggplot(data_NO_00, aes(Reference,RLM,color=Status)
      ...
      )+scale_color_manual(values=c("#000000","#ed553b")
      )+theme(axis.text = element_text(size = 8)
      )+theme(axis.title = element_text(size = 9)
      )+theme(plot.title = element_text(size = 10)
      )+guides(colour = guide_legend(override.aes = list(size=4)))+ theme(legend.position = "none")
     ...
     jpeg(paste("figure/2nd_colocation/scatterplot/",sensorunit,".jpeg",sep = ""), width =23, height = 11.5, units = 'cm', res = 600)
     multiplot(pdata_NO_A_RLM,pdata_NO2_A_RLM,
               pdata_NO_A_RF,pdata_NO2_A_RF,
               pdata_NO_B_RLM,pdata_NO2_B_RLM,
               pdata_NO_B_RF,pdata_NO2_B_RF,
               cols=4)
     dev.off()
